...
const app = new Hono<Env>()
  .get("/", async (c) => {
    ...
    const rows = await db.select().from(level).where(eq(level.fieldId, fieldId)).orderBy(level.sortOrder);
    ...
    await Promise.all(
      ids.map((id, i) =>
        db.update(level).set({ sortOrder: i, updatedAt: new Date() })
          .where(and(eq(level.id, id), eq(level.fieldId, fieldId))),
      ),
    );
  ...
  .put("/:entityId", zValidator("json", masterUpdateInputSchema), async (c) => {
    ...
    const [row] = await db.update(level).set(updates)
      .where(and(eq(level.id, c.req.param("entityId")), eq(level.fieldId, fieldId))).returning();
