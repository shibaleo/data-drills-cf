...
export default function ScopeDetailPage() {
  ...
  // 編集 state は useScopeEditState に集約。/plan と共有 (sync / dirty / save / handlers)。
  ...
  const edit = useScopeEditState({
    scopeId, data: data ?? null, today, realToday, asOf, allProblems,
  });
  const {
    name, setName,
    ...
    synced, planDirty, layersDirty, milestonesDirty, filterDirty, dirty,
    reset, save, isSaving,
    ...
    <div className="p-3 md:p-4 flex flex-col gap-2">
      ...
      <div className="rounded-md border p-3 space-y-2">
        ...
        <BacklogChart
          ...
          /* useScopeEditState の handlers を rest spread。
             readOnly (= snapshot 表示中) は編集系コールバックを渡さない → chart 側で
             cursor-grab・+ ボタン等の affordance も無効化される。 */
          {...(readOnly ? {} : {
            onMilestoneDateDraft: editHandlers.onMilestoneDateDraft,
            onMilestoneDateChange: editHandlers.onMilestoneDateChange,
            onMilestoneLayerDraft: editHandlers.onMilestoneLayerDraft,
            onMilestoneLayerChange: editHandlers.onMilestoneLayerChange,
            onMilestoneTargetChange: editHandlers.onMilestoneTargetChange,
            onMilestoneRemove: editHandlers.onMilestoneRemove,
            onMilestoneAddToLayer: (layerId: string, atDate?: string) =>
              editHandlers.onMilestoneAddToLayer(layerId, atDate, centerDate(), memberCount),
            onLayerNameChange: editHandlers.onLayerNameChange,
            onLayerColorChange: editHandlers.onLayerColorChange,
            onLayerStyleChange: editHandlers.onLayerStyleChange,
            onLayerRemove: editHandlers.onLayerRemove,
