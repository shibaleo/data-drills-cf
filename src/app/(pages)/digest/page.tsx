...
/**
 * /digest — ?scopeId= 駆動の入口。/review と同じ bridge パターン。
 */
...
export default function DigestEntryPage() {
  ...
  const search = useSearch({ from: "/digest" as never }) as { scopeId?: string };
  ...
  useEffect(() => {
    ...
    if (match) {
      ...
      navigate({ to: "/digest/$scopeId" as string, params: { scopeId: match.id }, replace: true });
    }
  }, [search.scopeId, digestScopes, isLoading, currentProject, navigate]);
  ...
  return (
    <div className="p-3 md:p-4 flex flex-col gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {digestScopes.map((s) => (
          <Link key={s.id} to="/digest/$scopeId" params={{ scopeId: s.id }}
